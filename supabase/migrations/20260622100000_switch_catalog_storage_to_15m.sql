-- Switch catalog storage timeframe from 5m to 15m.

update trading.signal_agents
set
  agent_id = 'ma-cross-15m-v1',
  allowed_timeframes = array['15m']::text[],
  description = 'Fast/slow simple moving average crossover on 15m closes; ENTER on bullish cross at bar close, else HOLD.',
  updated_at = now()
where agent_id = 'ma-cross-5m-v1';

update trading.signal_agents
set
  agent_id = 'rsi-reversion-15m-v1',
  allowed_timeframes = array['15m']::text[],
  description = 'RSI mean-reversion on 15m closes; ENTER when RSI crosses up from oversold, else HOLD.',
  updated_at = now()
where agent_id = 'rsi-reversion-5m-v1';

update trading.signal_agents
set
  agent_id = 'breakout-atr-15m-v1',
  allowed_timeframes = array['15m']::text[],
  description = 'Breakout with ATR filter on 15m closes; ENTER on qualified range break, else HOLD.',
  updated_at = now()
where agent_id = 'breakout-atr-5m-v1';

alter table trading.executors
  alter column moving_floor_timeframe set default '15m';

update trading.executors
set moving_floor_timeframe = '15m'
where moving_floor_timeframe = '5m';
...
create or replace function trading.executor_dashboard_pnl_snapshot(
  p_executor_id uuid,
  p_user_id uuid,
  p_catalog_timeframe text default '15m'
)
returns jsonb
language plpgsql
stable
...
comment on function trading.executor_dashboard_pnl_snapshot(uuid, uuid, text) is
  'PnL strip for executor detail: SUM filled buy notionals, position cost basis, mark from latest 15m catalog close per open market.';

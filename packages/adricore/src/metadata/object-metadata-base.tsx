...
import { baseObjectFieldRegistry } from "./standard-fields";
...
/** Apinames of fields auto-cloned by every base subclass. Excluded from default columns. */
const SYSTEM_FIELD_API_NAMES = new Set<string>([
  "id",
  "created_by",
  "created_at",
  "updated_by",
...
/** Options for `ObjectMetadataBase.toListPageHeaderProps` / `CreateListPageHeader`. */
...
/** Options for `ObjectMetadataBase.toDetailPageHeaderProps` / `CreateDetailPageHeader`. */
...
/**
 * Common base for every object metadata class.
 *
 * Holds the schema/table/apiName + label + icon + route, the standard audit
 * field registry (`id`, `created_by`, `created_at`, `updated_by`, `updated_at`),
 * and the routing/rendering helpers. Subclasses decide whether the row has a
 * user-facing `name`:
 *
 * - {@link ObjectMetadata} — adds a `name` standard field and a {@link NameFieldSpec}.
 * - {@link HighVolumeObjectMetadata} — no `name`, no name-field config; meant for
 *   append-only / high-volume tables (logs, candles, signal jobs, …).
 */
export abstract class ObjectMetadataBase implements AdriObjectMetadata {
  ...
  constructor(
    ...
    const pathSegment = apiName.replace(/_/g, "-");
    this.route = new RouteMetadata(this.icon, `/${pathSegment}`, this.label.plural);

    this.slug = pathSegment;
    this.idColumn = "id";

    this.fieldRegistry = baseObjectFieldRegistry.clone(this);
  }
   ...
   *
   * Base implementation falls back to `record.id` then `""` since the base
   * contract does not include a `name` column. {@link ObjectMetadata} overrides
   * this to honor its {@link NameFieldSpec}.
   */

...
/**
 * `select("supports_*") .from("v_exchange_capabilities") .eq("exchange_id", id) .maybeSingle()`
 * — capabilities-only narrow lookup. Reads from the
 * `catalog.v_exchange_capabilities` view since the exchange-level columns
 * were dropped by migration 20260724020000.
 */
...
/**
 * `select("exchange_id as id, supports_*") .from("v_exchange_capabilities")` —
 * all rows with capabilities (executor form options). Reads from the
 * `catalog.v_exchange_capabilities` view since the exchange-level columns
 * were dropped by migration 20260724020000.
 */
export async function selectAllCapabilities(
  ...
  const { data, error } = await client
    ...
    .from("v_exchange_capabilities")
    .select(
      "exchange_id, supports_spot_buy, supports_spot_sell, supports_margin_long, supports_margin_short",
    );
  if (error) throw new Error(error.message);
  return ((data ?? []) as {
    exchange_id: string;

...
import { runMarketBackfillSignals } from "@/lib/orchestrators/market-backfill-signals.service";
...
async function handle(request: Request, rawBody: string): Promise<Response> {
  ...
  try {
    const result = await runMarketBackfillSignals(admin, { marketId, startDate, endDate });
    return NextResponse.json(result);
  } catch (e) {
    const message = e instanceof Error ? e.message : "market backfill signals failed";
